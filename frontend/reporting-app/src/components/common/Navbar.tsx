      ...
      <div className="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden lg:flex items-center gap-2 bg-slate-100 rounded-lg p-1 text-xs border border-slate-200">
          ...
          <select
            ...
            className="bg-transparent font-semibold text-slate-900 focus:outline-hidden cursor-pointer pr-2"
          >
        ...
        {/* Right Section: AI Assistant, Quick Switcher, User Profile */}
        <div className="flex items-center gap-2.5 sm:gap-3">
          {/* AI Assistant Button */}
          <button
            type="button"
            onClick={onOpenAiAssistant}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-lg transition-colors shadow-xs"
          ...
          <div className="relative group">
            <button
              type="button"
              className="flex items-center gap-1.5 text-xs font-medium px-2.5 py-1.5 bg-slate-50 border border-slate-200 rounded-lg hover:bg-slate-100 text-slate-700 transition-colors"
            >
              <UserCheck className="h-3.5 w-3.5 text-indigo-600" />
              <span className="capitalize hidden md:inline">
                {currentUser.role === 'manager' ? 'Manager View' : 'Member View'}
              </span>
          ...
          <div className="flex items-center gap-2 pl-2 border-l border-slate-200">
            ...
            <div className="hidden xl:block text-left">
              ...
              <p className="text-[10px] text-slate-500 leading-tight capitalize">{currentUser.role.replace('_', ' ')}</p>
            </div>
            <button
              type="button"
              onClick={logout}
